...
const filterCards = (card) => request.get(urlGods(card.name), (res, err, body) => {
    try {
        const { result } = JSON.parse(body);
        if (result && result.length > 0) {
            if (result[0]?.buy && result[1]?.buy && parseCardValue(result[0].buy.data) / parseCardValue(result[1].buy.data) < 0.95) {
